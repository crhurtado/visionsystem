...
if (isset($_POST["contactEmail"])){
	...
	$contactLastN = htmlentities(substr($_POST["contactLastN"], 0, 200), ENT_QUOTES);	
	...
	if ($contactName == "" || $contactTelf == "" || $contactEmail == "" || $messageContent == "" || $contactLastN == ""){
		$er .= 'Tu nombre, apellido, telefono, correo electronico y mensaje no pueden quedar en blanco, por favor llenalos e intenta de nuevo.<br />';
	}
	...
	if ($er == ''){
		...
		$subject = $siteName . ' Solicitud de Presupuesto';
		$message = 
		'<html>
			<head>
			<title>' . $siteName . ': Solicitud de Presupuesto Online</title>
			</head>
			<body>
			<p> Mensaje enviado por: '.$contactLastN.', '.$contactName.'</p>
		...
		$to = $siteName . ' Presupuesto Online <' . $siteEmail . '>';
		...
		if (mail($to, $subject, $message, $headers)){
			echo '<div id="contactform">Gracias por escoger a '. $siteName .'. Nuestro equipo evaluará su solicitud y le contactará a la brevedad posible.</div>';
		}
		else {
...
if ($er != '' && isset($_POST["contactEmail"])){
	showContactForm($contactName, $contactLastN, $contactTelf, $contactEmail, $messageContent, $er);
}
...
function showContactForm($contactName = "", $contactLastN = "", $contactTelf = "", $contactEmail = "",  $messageContent = "", $er = ''){
	echo '
	<div id="contactform">
	...
	<form action="' . $_SERVER["REQUEST_URI"] . '" method="post" name="contactForm">		
			<ul>
				...
				<li>
					<label for="contactLastN">Apellido: </label>
					<input name="contactLastN" type="text" size="20" maxlength="200" value="' . $contactLastN . '" />
				</li>				
				<li>
					<label for="contactTelf">Teléfono: </label>
					<input name="contactTelf" type="text" size="20" maxlength="25" value="' . $contactTelf . '" />
				</li>
				<li>
				...
				<li>
					<label for="messageContent">Descripción: </label>
					<textarea name="messageContent" cols="35" rows="8">' . $messageContent . '</textarea>
				</li>
				
				<input name="submitButton" type="submit" value="Enviar Solicitud" />
			</ul>
